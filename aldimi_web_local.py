# aldimi_web_local.py — Version LOCAL para Windows con Tesseract + EasyOCR completo
# Este archivo se usa SOLO en tu PC para el OCR. Render usa aldimi_web.py (sin Tesseract).

import os, re, json, time, math, hashlib, datetime, logging, tempfile
import numpy as np

# ── Ruta de Tesseract para Windows ───────────────────────────────────────────
import pytesseract
...
def _extraer_texto_imagen(ruta):
    """
    Extrae texto de una imagen usando Tesseract primero (rápido),
    y EasyOCR como respaldo si Tesseract no detecta suficiente texto.
    Igual que la estrategia del archivo original de los compañeros.
    """
    texto = ''

    # --- Intento 1: Tesseract (más rápido, menos RAM) ---
    ...
    # --- Intento 2: EasyOCR (más preciso en imágenes difíciles) ---
    reader = _get_reader()
    if reader:
        try:
