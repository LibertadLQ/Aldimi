...
async function cargarEstadoInicial() {
  try {
    ...
    if (statPacientes) statPacientes.textContent = data.total ?? (data.pacientes || []).length;
    if (statDocumentos) statDocumentos.textContent = data.documentos ?? 0;
  } catch (err) {
    console.warn('No se pudo cargar el estado inicial de pacientes:', err);
  }
}
