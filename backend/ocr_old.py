...
_DNI_DB_PATH = Path(__file__).resolve().parent / "dni_conocidos.json"


def _cargar_dni_db() -> Dict[str, Any]:
    if not _DNI_DB_PATH.exists():
        return {}
    try:
        with open(_DNI_DB_PATH, "r", encoding="utf-8") as f:
            data = json.load(f)
            return data if isinstance(data, dict) else {}
    except Exception:
        return {}

_DNI_DB_CONOCIDOS = _cargar_dni_db()
...
def _preprocesar_imagen(ruta_imagen: str):
    ...
    coords = cv2.findNonZero(255 - umbral)
    if coords is not None and len(coords) > 0:
        ...
        if abs(angle) > 1.0:
            ...
            umbral = cv2.warpAffine(umbral, M, (w, h), flags=cv2.INTER_CUBIC, borderMode=cv2.BORDER_REPLICATE)
...
def extraer_texto_ocr(ruta_imagen: str, lang: str = "spa+eng", psm: int = 6) -> str:
    ...
    processed = _preprocesar_imagen(ruta_imagen)
...
def extraer_ciu_dni(texto: str, tipo_dni: str) -> Optional[str]:
    ...
    m = re.search(r"\bDNI\s*[:\-]?\s*(\d{8})\b", t, re.I)
    if m:
        return _fix_num(m.group(1))
...
def extraer_campos_lab(texto: str) -> Dict[str, Any]:
    ...
    for linea in (texto or "").split("\n"):
        ...
        m_widal = _WIDAL_RE.search(ls)
        if m_widal:
            numerador, denominador = m_widal.groups()
            try:
                num = int(numerador)
                den = int(denominador)
                nombre = "Widal (ratio)"
                ...
                if key not in seen:
                    ...
                    prueba = {
                        ...
                        "unidad": "ratio",
                        "referencia": "<1:40",
                        "flag": "H" if num >= 40 or den >= 40 else "",
                    }
        ...
        m_qual = _QUALITATIVE_RE.match(ls)
        if m_qual:
            nombre_raw, resultado_raw = m_qual.groups()
            nombre = _norm_nombre_lab(nombre_raw.strip())
            resultado = resultado_raw.strip().upper()
            key = nombre.lower()
            if key not in seen:
                seen.add(key)
                flag = ""
                if resultado in ("POSITIVO", "DETECTADO", "PRESENTE", "SENSITIVE", "SENSIBLE"):
                    flag = "H"
                elif resultado in ("NEGATIVO", "NO DETECTADO", "AUSENTE", "RESISTANT", "RESISTENTE"):
                    flag = "L"
                ...
                prueba = {
                    ...
                    "valor": resultado,
                    "unidad": "",
                    "referencia": "Negativo/No Detectado",
                    "flag": flag,
                }
                res["pruebas"].append(prueba)
                if flag == "H":
