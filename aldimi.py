# aldimi.py — ARCHIVO ÚNICO de producción + desarrollo
# Fusión de aldimi_web_local.py (Tesseract + EasyOCR) y aldimi_web.py (solo EasyOCR)
# Selecciona automáticamente según ALDIMI_ENV (local/prod):
#  - ALDIMI_ENV='local' o ausente → Tesseract (Windows) + EasyOCR (respaldo)
#  - ALDIMI_ENV='prod'/'production' → Solo EasyOCR, /tmp/ALDIMI_DB para persistencia
...
if _USE_TESSERACT:
    # Desarrollo local: usar home directory
    DB_FOLDER = os.environ.get('ALDIMI_DB_PATH', os.path.join(os.path.expanduser('~'), 'ALDIMI_DB'))
else:
    # Producción (Render, etc.): usar /tmp
...
# ── Tesseract: solo en desarrollo (si está disponible) ──────────────────────
_TESSERACT_OK = False
if _USE_TESSERACT:
    try:
        import pytesseract
        ...
        print(f'✗ Tesseract no disponible (desarrollo sin Tesseract): {e}')
        _TESSERACT_OK = False
else:
    print(f'ℹ Entorno producción ({ALDIMI_ENV}): Tesseract deshabilitado')
...
def _get_reader():
    """Retorna el reader de EasyOCR (singleton)."""
    global _easyocr_reader
    if _easyocr_reader is None and _EASYOCR_OK:
        try:
            langs = ['es', 'en'] if _USE_TESSERACT else ['es']
            _easyocr_reader = _easyocr_lib.Reader(langs, gpu=False, verbose=False)
...
def _extraer_texto_imagen(ruta):
    ...
    Si _USE_TESSERACT: intenta Tesseract primero, luego EasyOCR como respaldo.
    Si no: usa solo EasyOCR.
    """
    texto = ''
...
# Cargar BD al importar el módulo
cargar_bd()
print(f'✓ aldimi.py cargado ({ALDIMI_ENV} mode)')
